...
function EditBook() {
  ...
  useEffect(() => {
    ...
    const fetchBook = async () => {
      const repsonse = await axios.get(BACKEND_ENDPOINT + `books/?_id=${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });

      setExistingbook(repsonse.data[0]);
    };
  ...
  useEffect(() => {
    if (existingBook) {
      setFormData((prevState) => ({
        ...
        publishDate: existingBook.publishDate.replace("T00:00:00.000Z", ""),
    ...
    try {
      ...
      const response = await axios.put(
        ...
        // formDat,
        {
          ...
          tags: formData.tags,
          author: authorId,
          cover: avatar ? avatar : existingBook.cover,
        },
        {
          headers: {
            Authorization: `Bearer ${token}`,
